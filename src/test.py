...
def create_model(actions, sampling=False):
  ...
  model.source_seq_len = args.seq_length_in if not sampling else 50
  model.target_seq_len = args.seq_length_out if not sampling else 100
  return model
